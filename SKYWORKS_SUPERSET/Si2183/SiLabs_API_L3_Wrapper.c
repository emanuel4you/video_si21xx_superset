...
/*                                  Skyworks API Functions                                                   */
/*-----------------------------------------------------------------------------------------------------------*/
/*   This source code contains all API functions for a Skyworks digital TV demodulator Evaluation board      */
/*-----------------------------------------------------------------------------------------------------------*/
/* Change log: */
/* Last  changes:
...
/* Older changes:
  ...
  As from V2.5.2:
    ...
    <improvement>[Duals/SiLabs_EVBs] In SiLabs_API_SW_Init: surrounding ts_mux related code by USB_Capability, since it's only valid on some Skyworks EVBs
  ...
  As from V2.4.3:
   <improvement> In SiLabs_API_TS_Mode: removing TS parallel shape setup for Si2183.
                  ...
                  GPIF mode is only used with Skyworks EVBs and is never used together with serial or parallel.
  ...
  As from V2.3.5:
    <new_feature> [Si2164/SPI) Adding SiLabs_API_SPI_Setup (Only available with Si2164 derivatives as from today)
       ...
       CAUTION2: When used with Skyworks EVBs, this requires updating the following items to support SPI download using the Cypress chip:
        - Cypress FW
        - Cypress DLL
  ...
  As from V2.0.4:
   ...
   In SiLabs_API_Demod_status:
    Added Si2167 compatibility with TER tuners not from Skyworks
...
L0_Context  WrapperI2C_context;
       ...
       /* Translation functions from 'Custom' values to 'Skyworks' values */
#if 1
...
#endif /* Translation functions from 'Custom' values to 'Skyworks' values  */
       /* Translation functions from 'Skyworks' values to 'Custom' values */
#if 1
/************************************************************************************************************************
...
#endif /* Translation functions from 'Skyworks' values to 'Custom' values */
...
/************************************************************************************************************************
  ...
  Use:        rate computing function
              Used to fill the integer/decimal/exponent parts of a scientific-displayed rate depending on
               the mant and exp values returned by the Skyworks API functions (used for BER/PER/FER display)
...
/*****************************************************************************************/
/*               Skyworks demodulator API functions (demod and tuner)                      */
/*****************************************************************************************/
  ...
  if (front_end->chip ==   0x2183 ) {
    switch (ts_mode) {
      ...
      case SILABS_TS_PARALLEL: { front_end->Si2183_FE->demod->prop->dd_ts_mode.mode = Si2183_DD_TS_MODE_PROP_MODE_PARALLEL;
                                  ...
                                  L0_Cypress_Process ("initdata", "gpifctlcfg", 128.0, &retdval, &rettxt); /* On Skyworks EVBs, turn Cypress CTL pins tristate */
#endif /* USB_Capability */
                                 break; }
...
/************************************************************************************************************************
  ...
  DESCRIPTION: Skyworks API information function used to retrieve the version information of the Skyworks API wrapper
  Returns:    the Skyworks API version information string
************************************************************************************************************************/
