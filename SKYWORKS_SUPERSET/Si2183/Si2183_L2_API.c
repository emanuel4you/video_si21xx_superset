 ...
 *************************************************************************************************************/
/***************************************************************************************
                  Skyworks Solutions Broadcast Si2183 Layer 2 API
...
/* Change log:
 As from V0.3.6.0:
    <firmware> With FW 6_0b17 on x60 parts
...
/* Older changes:
 ...
 As from V0.1.0.0:
    ...
    This assumes that the TER tuner address is 0xC0, to match Skyworks EVBs.
 ...
 As from V0.0.1.0:
    ...
    <new_feature>[TER_Tuner/Config] In Si2183_L2_switch_to_standard:
     ...
     NB: No change required for existing applications, since this is mostly useful to use LIF_IF1 with Skyworks TER tuners,
 ...
 As from V0.0.0.4:
    ...
    <improvement>[TER_BW/1.7MHz] In Si2183_L2_Tune: now using SILABS_BW enum as defined in SiLABS_TER_TUNER API V0.3.9, to
      use the 1.7 MHz filtering feature in Skyworks TER tuners whenever possible.
...
#endif /* CHANNEL_BONDING */
#ifndef   CHANNEL_BONDING
 #include "Si2183_ROM2_Patch16_6_0b17.h"     /* firmware compatible with Si2183     (ROM2/NVM 6_0b1) without   channel bonding            */
 #define   Si2183_FIRMWARE_FOR_x60_PARTS           Si2183_PATCH16_6_0b17
#endif /* CHANNEL_BONDING */
/* SPI FW download */
...
//  #define   SPI_DEBUGGING
    ...
    /* <porting> For Skyworks EVB, no call to L0_EnableSPI is necessary
...
signed   int  Si2183_L2_Infos              (Si2183_L2_Context *front_end, char *infoString)
{
    ...
    STRING_APPEND_SAFE(infoString,1000, "Terrestrial tuner    Skyworks    at 0x%02x\n", front_end->tuner_ter->i2c->address);
...
/************************************************************************************************************************
  ...
  Use:        Front-End connection function
              Specific to Skyworks USB connection!
